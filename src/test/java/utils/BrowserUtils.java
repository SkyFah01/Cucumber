...
public class BrowserUtils {
    ...
    public static void getScreenShortForCucumber(WebDriver driver , Scenario scenario){
        Date currentDate = new Date();
          String screenShortFileName = currentDate.toString().replace(" ","-")
                  .replace(":","-");
          if(scenario.isFailed()){
              File screenShortFile =((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
              try {
                  FileUtils.copyFile(screenShortFile,new File("src/test/java/screenshot/"+screenShortFileName+".png"));
              } catch (IOException e) {
                  throw new RuntimeException(e);
              }
          }
